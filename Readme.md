...
После чего привести конфигурационные файлы к определенному виду и на сервере и на клиенте для того чтобы все заработало. На клиенте также необходимо сделать так чтобы агент стартовал автоматически. Конфигурационные файлы во вложении. Пароли, в конфигурационных файлах должны быть одинаковыми на клиенте и на сервере, т.е. чтобы подключиться клиенту к серверу, необходим правильный пароль например указан к директору.
